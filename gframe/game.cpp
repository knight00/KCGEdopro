	...
	switch (rule)
	{
		...
		case MODE_RULE_ZCG_NO_RANDOM:
			CreateGame(cscg,{ {0,999},{0,999},{0,999} },5,0,5,32000,1,223,0,0,858112 & 0xffffffff,(858112 >> 32) & 0xffffffff,
				true,false,SERVER_HANDSHAKE,{ EXPAND_VERSION(CLIENT_VERSION) },1,1,1,67108864,0);
			break;
		case MODE_RULE_5DS_DARK_TUNER:
			CreateGame(cscg,{ {0,999},{0,999},{0,999} },5,0,5,8000,1,600,0,0,858112 & 0xffffffff,(858112 >> 32) & 0xffffffff,
				true,false,SERVER_HANDSHAKE,{ EXPAND_VERSION(CLIENT_VERSION) },1,1,1,67108864,0);
			break;
		default:
			break;
