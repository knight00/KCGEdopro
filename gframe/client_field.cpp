...
void ClientField::ShowSelectCard(bool buttonok, bool chain) {
	...
	for(size_t i = 0; i < ct; ++i) {
		...
		//////////kdiy/////////
		mainGame->selectedcard[i]->setRelativePosition(mainGame->Scale<irr::s32>(static_cast<irr::s32>(startpos - 10 + i * 125), 55, static_cast<irr::s32>(startpos + 10 + i * 125), 75));
		//////////kdiy/////////
	}
	if(selectable_cards.size() <= 5) {
...
void ClientField::GetCardDrawCoordinates(ClientCard* pcard, irr::core::vector3df* t, irr::core::vector3df* r, bool setTrans) {
	...
	if(location != LOCATION_HAND) {
		...
		//if(location == LOCATION_MZONE) {
			// if(controler == 0)
		if((location == LOCATION_MZONE && !(pcard->status & STATUS_SMZONE)) || (location == LOCATION_SZONE && (pcard->status & STATUS_MSZONE))) {
		...
		////////kdiy///////////
            ...
            *r = ((pcard->overlayTarget->controler == 0 || pcard->is_attack) && !pcard->attackopp) ? selfATK : oppoATK;
            ////////kdiy///////////
		else
		...
		if(((location & (LOCATION_GRAVE | LOCATION_OVERLAY)) == 0) && ((location == LOCATION_DECK && deck_reversed == pcard->is_reversed) ||
			...
			*r += facedown;
			...
			if(((location == LOCATION_MZONE && !(pcard->status & STATUS_SMZONE)) || (location == LOCATION_SZONE && (pcard->status & STATUS_MSZONE))) && (pcard->position & POS_DEFENSE))
			////////kdiy///////////
