...
bool GenericDuel::IteratePlayers(std::function<bool(duelist& dueler)> func) {
	for(auto& dueler : players.home) {
		if(dueler.player)
			if(!func(dueler))
				return false;
	...
	if(match_result.empty()) {
		...
		if(deckManager.LoadSide(dueler.pdeck, (int*)deckbuf, mainc, sidec)) {
			...
			if(CheckReady()) {
				...
				IteratePlayers([&player](duelist& dueler) {
...
void GenericDuel::StartDuel(DuelPlayer* dp) {
	if(dp != host_player)
		return;
	if(!IteratePlayers(std::function<bool(duelist&)>([](auto& dueler)->bool {
		if(!dueler.ready)
			return false;
		return true;
	})))
...
int GenericDuel::Analyze(CoreUtils::Packet packet) {
	...
	if(record)
		replay_stream.insert(record_last ? replay_stream.end() : replay_stream.begin(), packetcpy);
	new_replay.WriteStream(replay_stream);
	new_replay.Flush();
	if(return_value == 1) {
